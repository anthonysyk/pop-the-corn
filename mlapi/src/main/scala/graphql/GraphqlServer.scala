...
import contentbased.RunContentBasedRecommendation
...
object GraphqlServer {
  ...
  def main(args: Array[String]): Unit = {
    ...
    RunContentBasedRecommendation.computeMoviesState()
        ...
        path("profile") {
          post {
            entity(as[String]) { json =>
              ...
              complete(userProfile.map(RunContentBasedRecommendation.computeUserProfileRecommendation).getOrElse(Nil).asJson.noSpaces)
            }
          }
        }
